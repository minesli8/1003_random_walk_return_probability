...
def returned(n):
    ...
    for i in range(1, M):
        ...
        temp[direction[0]] = pos[i-1][direction[0]] + d
        for j in range(n):
            if(j!=direction[0]):
                temp[j]=pos[i-1][j]             
...
if __name__ == '__main__':
    n_simulation = int(1e4)

    start_time = time.time()

    report = open('./results_parallel.txt', 'w')
    ...
    count = sum(flags)            
    print('For 1-dimension, the probability is:')
    p = count/n_simulation
    print(p)
    report.write('For 1-dimension, the probability is:\n')
    report.write('%f\n' %(p))
    ...
    count = sum(flags)            
    print('For 3-dimension, the probability is:')
    p = count/n_simulation
    print(p)
    report.write('For 3-dimension, the probability is:\n')
    report.write('%f\n' %(p))
